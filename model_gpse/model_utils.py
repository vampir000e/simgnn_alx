...
class SiameseDataSet(object):
    def __init__(self, data_save_folder, data_set_name, validation_ratio, node_feat_name, node_encoder_name):
        ...
        # print('\t\ttrain_gs.len={} and val_gs.len={}, and their max number of nodes = {}'.format(len(self.train_gs),len(self.val_gs),self.train_val_max_number_of_nodes))
        ...
        # print('\t\ttest_gs.len={}, and their max number of nodes = {}'.format(len(self.test_gs),self.test_max_number_of_nodes))
    ...
    def load_data_set(self, data, train_or_test):
        if data == 'AIDS700nef':
            ...
            return PTCData(save_folder=self.data_save_folder, train_or_test=train_or_test)
        elif data == 'IMDBMulti':
            return IMDBData(save_folder=self.data_save_folder, train_or_test=train_or_test)
        else:
...
class AIDSData(Data):
    def __init__(self, save_folder, train_or_test):
        ...
        data_dir = '{}/{}'.format(self.get_save_folder(), train_or_test)
        # print(data_dir)
        ...
        # print('The max number of nodes among all graphs of {} is # {}'.format(self.get_dataset_folder_name(),self.get_max_number_of_nodes()))
...
class PTCData(Data):
    def __init__(self, save_folder, train_or_test):
        super(PTCData, self).__init__(save_folder=save_folder)
        # Load all graphs
        data_dir = '{}/{}'.format(self.get_save_folder(), train_or_test)
        self.graphs = []
        self.graphs = iterate_get_graphs(data_dir)
...
class Result(object):
    """
    The figures object loads and stores the ranking figures of a model
        for evaluation.
        Terminology:
    ...
    def dist_sim_mat(self, norm):
        """
        Each figures object stores either a distance matrix
    ...
    def sort_id_mat(self, norm):
        """
        :param norm:
        :return: a m by n matrix representing the ranking figures.
...
class DistanceModelResult(Result):
    ...
    def _load_result_mat(self, metric, model, m, n):
        ...
        # file = '../results' + '/{}/{}/{}'.format(self.datasets, metric, 'ged_ged_mat_aids700nef_astar_2018-06-27T19_38_42_qilin_10cpus.npy')
        return np.load(file)
